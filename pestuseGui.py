...
#                    self.ui.tableWidget_ateliersYZ.item(iy, 0).setTextAlignment(QtCore.Qt.AlignCenter)
        ...
        # Decision
        # On rend desabled le tableau des ateliers YZ pour eviter la saisie a l interieur de ses cases
        self.ui.tableWidget_ateliersYZ.setEnabled(False)
        # On rend desabled les tableaux historiques pour eviter la saisie a l interieur
        if periode == 1:
            self.ui.tableWidget_histo_produitY.setEnabled(False)
            self.ui.tableWidget_histo_produitZ.setEnabled(False)
        else:
            self.ui.tableWidget_histo_produitY.setEnabled(True)
            self.ui.tableWidget_histo_produitZ.setEnabled(True)
...
#        self.setFixedSize(1032, 768)
    ...
    def calcul_profit_Y(self, le_rendement_Y,  le_nbX_pour_Y):
        ...
        if le_nbX_pour_Y == 0:
            ...
            profitY = pms.BONUS + 10 * le_rendement_Y - le_nbX_pour_Y
            if self.type_partie != "BEN":
                profitY += pms.MONTANT_FIXE
    ...
    def ini_graph_rendement_Y(self, x_du_point):    
        ...
        # On ne trace le graphique qu a partir de 1 car 0 n est pas calculable
        self.x = plab.linspace(pms.MIN_X_POUR_Y + 1, pms.MAX_X_POUR_Y, 50)
    ...
    def ini_graph_profit_Y(self, x_du_point):
        ...
        if self.type_partie != "BEN":
            self.y = pms.BONUS + 10 * rend_Y - self.x + pms.MONTANT_FIXE
        else:
            self.y = pms.BONUS + 10 * rend_Y - self.x
...
#        y_du_point = pms.BONUS + 10 * rend_y_du_point - x_du_point
        y_du_point = 0
    ...
    def maj_graph_profit_Y(self, x_du_point):
        ...
        rend_y_du_point = 20.0 * x_du_point ** .3
        if self.type_partie != "BEN":
            y_du_point = pms.BONUS + 10 * rend_y_du_point - x_du_point + pms.MONTANT_FIXE
        else:
            y_du_point = pms.BONUS + 10 * rend_y_du_point - x_du_point
...
#        self.graph_rendement_Y(rendementY, nbX_pour_Y)
    ...
    def calcul_profit_Z(self, le_rendement_Z_max,  le_rendement_Z_moy,  le_rendement_Z_min,  le_nbX_pour_Z):
        ...
        if le_nbX_pour_Z == 0:
            ...
            if self.type_partie != "BEN":
                profitZ_max = pms.BONUS + 10 * le_rendement_Z_max - le_nbX_pour_Z + pms.MONTANT_FIXE
                profitZ_moy = pms.BONUS + 10 * le_rendement_Z_moy - le_nbX_pour_Z + pms.MONTANT_FIXE
                profitZ_min = pms.BONUS + 10 * le_rendement_Z_min - le_nbX_pour_Z + pms.MONTANT_FIXE
            else:
                profitZ_max = pms.BONUS + 10 * le_rendement_Z_max - le_nbX_pour_Z
                profitZ_moy = pms.BONUS + 10 * le_rendement_Z_moy - le_nbX_pour_Z
                profitZ_min = pms.BONUS + 10 * le_rendement_Z_min - le_nbX_pour_Z
    ...
    def ini_graph_rendement_Z(self, x_du_point):    # NOUVELLE APPROCHE TEST
        ...
        # On ne trace le graphique qu a partir de 1 car 0 n est pas calculable
        self.x = plab.linspace(pms.MIN_X_POUR_Z + 1, pms.MAX_X_POUR_Z, 600)
        if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
            self.y_moy = (20.0 * self.x ** .3) - pms.ASSUR
        else:
            self.y_moy = (20.0 * self.x ** .3)
        self.line, = self.axes_rdt_Z.plot(self.x, self.y_moy)
        if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
            self.y_max = (20.0 * self.x ** .3)  + (1 * 100 * self.x ** pms.BETA) - pms.ASSUR
        else:
            self.y_max = (20.0 * self.x ** .3)  + (1 * 100 * self.x ** pms.BETA)
        self.line, = self.axes_rdt_Z.plot(self.x, self.y_max)
        if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
            self.y_min = (20.0 * self.x ** .3)  - (1 * 100 * self.x ** pms.BETA) - pms.ASSUR + pms.INDEMNITE
        else:
        ...
        if x_du_point == 0:
            y_du_point_max = 0
            self.point = self.axes_rdt_Z.scatter([0, ], [y_du_point_max, ], 50, color='black')
            y_du_point_moy = 0
            self.point = self.axes_rdt_Z.scatter([0, ], [y_du_point_moy, ], 50, color='black')
            y_du_point_min = 0
            self.point = self.axes_rdt_Z.scatter([0, ], [y_du_point_min, ], 50, color='black')
        else:
            if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
                y_du_point_max = (20.0 * x_du_point ** .3)  + (1 * 100 * x_du_point ** pms.BETA) - pms.ASSUR
            else:
            ...
            if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
                y_du_point_moy = 20.0 * x_du_point ** .3 - pms.ASSUR
            else:
                y_du_point_moy = 20.0 * x_du_point ** .3
            self.point = self.axes_rdt_Z.scatter([x_du_point, ], [y_du_point_moy, ], 50, color='black')
            if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
                y_du_point_min = (20.0 * x_du_point ** .3)  - (1 * 100 * x_du_point ** pms.BETA) - pms.ASSUR + pms.INDEMNITE
            else:
    ...
    def ini_graph_profit_Z(self, x_du_point):
        ...
        if x_du_point == 0:
            ...
            if self.type_partie != "BEN":
                z_du_point_max = pms.BONUS + 10 * rend_z_du_point_max - x_du_point + pms.MONTANT_FIXE
            else:
                z_du_point_max = pms.BONUS + 10 * rend_z_du_point_max - x_du_point 
            self.point = self.axes_pro_Z.scatter([x_du_point, ], [z_du_point_max, ], 50, color='black')
            rend_z_du_point_moy = (20.0 * x_du_point ** .3)
            if self.type_partie != "BEN":
                z_du_point_moy = pms.BONUS + 10 * rend_z_du_point_moy - x_du_point + pms.MONTANT_FIXE
            else:
                z_du_point_moy = pms.BONUS + 10 * rend_z_du_point_moy - x_du_point 
            ...
            if self.type_partie != "BEN":
                z_du_point_min = pms.BONUS + 10 * rend_z_du_point_min - x_du_point + pms.MONTANT_FIXE
            else:
                z_du_point_min = pms.BONUS + 10 * rend_z_du_point_min - x_du_point 
    ...
    def maj_graph_profit_Z(self, x_du_point):
        ...
        if x_du_point == 0:
            rend_z_du_point_max = 0
            z_du_point_max = 0
            self.point = self.axes_pro_Z.scatter([0, ], [z_du_point_max, ], 50, color='black')
            rend_z_du_point_moy = 0
            z_du_point_moy = 0
            self.point = self.axes_pro_Z.scatter([0, ], [z_du_point_moy, ], 50, color='black')
            rend_z_du_point_min = 0
            z_du_point_min = 0
            self.point = self.axes_pro_Z.scatter([0, ], [z_du_point_min, ], 50, color='black')            
        else:
            rend_z_du_point_max = (20.0 * x_du_point ** .3) + (1 * 100 * x_du_point ** pms.BETA)
            if self.type_partie != "BEN":
                z_du_point_max = pms.BONUS + 10 * rend_z_du_point_max - x_du_point + pms.MONTANT_FIXE
    ...
    def calcul_rendement_Z(self, nbX_pour_Z):
        ...
        if nbX_pour_Z == 0:
            ...
            if self.type_partie == "WIN" or self.type_partie == "WEI" or self.type_partie == "WIE":
                rendementZ_max = (20.0 * nbX_pour_Z ** .3) + (1 * 100 * nbX_pour_Z ** pms.BETA) - pms.ASSUR
                rendementZ_moy = (20.0 * nbX_pour_Z ** .3) - pms.ASSUR
                rendementZ_min = (20.0 * nbX_pour_Z ** .3) - (1 * 100 * nbX_pour_Z ** pms.BETA) - pms.ASSUR + pms.INDEMNITE
            else:
